...
export function $3dTilesCulling(node, camera, displayDate) {
    // The temporal for tile.
    if (node.boundingVolume.yearOfConstruction > displayDate || node.boundingVolume.yearOfDemolition < displayDate) {
...
export function process3dTilesNode(cullingTest, subdivisionTest) {
    return function _process3dTilesNodes(context, layer, node) {
        ...
        // do proper culling
        const isVisible = cullingTest ? (!cullingTest(node, context.camera, this.displayDate)) : true;
